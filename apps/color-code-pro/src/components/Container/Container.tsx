...
export function Container() {
  ...
  const [lang, setLang] = useState(() => localStorage.getItem('lang') || 'typescript')

  const [copyButtonIconToUse, setCopyButtonIconToUse] = useState(<CopyIcon size={14} />)
  ...
  const handleLangChange = useCallback(
    (newLang: string) => {
      if (textRef.current) {
        ...
        textRef.current.value = savedCode.current[newLang] ?? LANG_PLACEHOLDER[newLang] ?? ''
      }
      setLang(newLang)
    },
    [lang],
  ...
  const currentMode = THEME_MODES.find(m => m.value === theme) ?? THEME_MODES[0]
  ...
  const doHighlight = useCallback(
    async (code: string) => {
      ...
      const html = await highlightCode(code, lang, activeTheme as ThemeRegistration)
  ...
  const copy = useCallback(async () => {
    ...
    if (theme === 'dual') {
      ...
      const html = await highlightCode(code, lang, themePair[scheme] as ThemeRegistration)
      if (!html) return
    ...
    <div className="cc-card">
      ...
      <div className="cc-header">
        ...
        <div className="cc-header-right">
          {/* Palette kind */}
          <PaletteKindSelect paletteKind={paletteKind} setPaletteKind={setPaletteKind} />
          ...
          <IconButton variant="ghost" aria-label={`Theme: ${currentMode.label} — click to cycle`} onClick={cycleTheme}>
            <ModeIcon size={14} weight={currentMode.value === 'dual' ? 'fill' : 'regular'} />
          </IconButton>
      ...
      <div className="cc-footer">
        <div className="cc-footer-left">
          ...
          {formatSupported && (
            <>
              ...
              <IconButton variant="ghost" aria-label="Format" onClick={handleFormat}>
        ...
        <div className="cc-footer-right">
          {mode === 'snippet' ? (
            <Button variant="primary" icon={copyButtonIconToUse} aria-label="Copy embed snippet" onClick={copy}>
              Copy Snippet
            </Button>
          ) : (
            <ThemeDownload />
...
const codeBlock = (lang: string, shikiHtml: string, { fg, outline, bg }: FrameColors) => {
  ...
  const inlinePreStyles = (html: string) => html.replace(/(<pre\b[^>]*\bstyle=")/, `$1${PRE_INLINE}`)
