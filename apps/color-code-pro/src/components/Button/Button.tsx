...
export function Button({ variant = 'default', icon, children, className, content, ...props }: ButtonProps) {
  const cls = [
    'cc-btn',
...
export function IconButton({ variant = 'default', children, className, ...props }: IconButtonProps) {
  const cls = [variant === 'ghost' ? 'cc-btn-icon-ghost' : 'cc-btn-icon', className].filter(Boolean).join(' ')
  return (
    <button className={cls} {...props}>
      {children}
    </button>
