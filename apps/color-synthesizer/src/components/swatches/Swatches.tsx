...
const UI_LAYOUT: string[] = [
  ...
  'surface',
  ...Array(23).fill('container'),
  'surface',
  'container',
  ...Array(9).fill('on-surface'),
  'container',
  ...Array(11).fill('container-sunken'),
  'container',
  'surface',
  ...Array(11).fill('container'),
  ...Array(2).fill('container-sunken'),
  'outline',
  ...Array(8).fill('container-sunken'),
  'container',
  'surface',
  'container',
  ...Array(7).fill('on-surface-variant'),
  ...Array(3).fill('container'),
  'container-sunken',
  ...Array(3).fill('outline'),
  ...Array(3).fill('container-sunken'),
  'outline',
  ...Array(3).fill('container-sunken'),
  'container',
  'surface',
  'container',
  ...Array(5).fill('on-surface-variant'),
  ...Array(5).fill('container'),
  ...Array(5).fill('outline'),
  ...Array(2).fill('container-sunken'),
  ...Array(3).fill('outline'),
  'container-sunken',
  'container',
  'surface',
  ...Array(11).fill('container'),
  ...Array(6).fill('outline'),
  'container-sunken',
  ...Array(4).fill('outline'),
  'container',
  'surface',
  ...Array(11).fill('container'),
  ...Array(11).fill('outline'),
  'container',
  'surface',
  'container',
...
export const Swatches = memo(function Swatches({
  ...
  useEffect(() => {
    if (
      ...
      (paletteKey === previousPaletteKeyRef.current && context.mode === previousModeRef.current)
    )
      return
      ...
      {Array.from({ length: totalSquares }).map((_, i) => {
        ...
        if (context.mode === 'ui') {
          ...
          if (colorSpace.format !== 'hex') {
            try {
              colorValue = new Color(hex).toString({ format: colorSpace.format as any, precision: 3 })
            } catch {
              /* keep hex */
            }
          }
          return <div key={i} className="swatch" data-color={colorValue} style={{ backgroundColor: hex }} />
        } else {
          const col = i % columns
          let colorIdx = Math.floor(col / columnsPerColor)
          if (colorIdx >= colorsCount) colorIdx = colorsCount - 1
