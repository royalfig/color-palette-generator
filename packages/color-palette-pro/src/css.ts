...
import { generateElevationShadowVars } from './ui'
...
export function generateCssVariables(palette: BaseColorData[], options: GenerateCssOptions): string {
  const { format, isUiMode = false, wrapper = 'none', shadows = true } = options

  let css = palette.map((color, idx) => {
  ...
  if (isUiMode && shadows) {
    ...
    if (primary) {
      ...
      const shadowVars = generateElevationShadowVars(primary, isDarkMode, format)
      css += '\n' + Object.entries(shadowVars).map(([k, v]) => `${k}: ${v};`).join('\n')
    }
  }
