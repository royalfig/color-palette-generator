...
import { adaptLightnessForText, adaptLightnessForQuiet, shiftHue, tintTowardHue, boostChroma } from '../utils'

/**
 * Triadic — three hues at ~120°. Balanced contrast in three directions.
 * Swatch layout: c0=base, c1=dark base, c2=triad-1 pure, c3=triad-1 muted, c4=triad-2 pure, c5=triad-2 muted.
 */
export const triadicTemplate: CodeThemeTemplate = {
  displayName: 'Triadic Balance',
  ...
  deriveColors(baseColor: Color, palette: BaseColorData[], isDarkMode: boolean, surfaces: SurfaceBundle): SyntaxColors {
    ...
    const regexColor = adaptLightnessForText(shiftHue(c4, -15), isDarkMode)
    const accentColor = adaptLightnessForText(boostChroma(c0.clone(), 1.2), isDarkMode)
