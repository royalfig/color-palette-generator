 ...
 */
export function intensityChromaFor(seedChroma: number, style: PaletteStyle): number {
  const t = Math.max(0, Math.min(1, Math.pow(seedChroma / 0.22, 0.85)))
  return Math.min(ANSI_C_CEILING, (ANSI_C_BAND[0] + (ANSI_C_BAND[1] - ANSI_C_BAND[0]) * t) * SHAPE_INTENSITY[style])
}
