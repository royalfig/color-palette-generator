...
export function generateAnalogous(
  ...
  try {
    ...
    if (isAchromatic(baseColorObj))
      return generateNeutralPalette(baseColor, 6, "analogous", format);
    ...
    if (style === "triangle") {
      ...
      const lightnessAdaptation =
        baseLightness < 0.4 ? 0.2 : baseLightness > 0.6 ? -0.2 : 0;
    ...
    const initialColors = analogousHues.map((hue, index) => {
      ...
      let finalChroma =
        (baseColorObj.oklch.c ?? 0) * variation.c * chromaAdjust;
    ...
    throw new Error(
      `Failed to generate analogous colors for ${baseColor}: ${e}`,
    );
  }
}
