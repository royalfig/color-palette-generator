...
export function generateTintsAndShades(
  ...
  try {
    ...
    const baseIndex = findBaseColorPosition(
      baseLightness,
    ...
    if (style === "square") {
      ...
      colors.push(
        ...generateSquareStyle(baseColorObj, lightnessProgression, baseIndex),
      );
    } else if (style === "triangle") {
      // Perceptual compensation for Bezold-Brücke and Abney effects
      colors.push(
        ...generateTriangleStyle(baseColorObj, lightnessProgression, baseIndex),
      );
    } else if (style === "circle") {
      // Chroma storytelling - dark=rich, light=ethereal
      colors.push(
        ...generateCircleStyle(baseColorObj, lightnessProgression, baseIndex),
      );
    } else if (style === "diamond") {
      // Tonal variations using color mixing
      colors.push(
        ...generateDiamondStyle(baseColorObj, lightnessProgression, baseIndex),
      );
    }
    ...
    throw new Error(
      `Failed to generate tints and shades for ${baseColor}: ${e}`,
    );
  }
}

// Square Style: Pure numerical consistency
function generateSquareStyle(
  baseColor: Color,
  lightnesses: number[],
  baseIndex: number,
): Color[] {
...
function generateTriangleStyle(
  baseColor: Color,
  lightnesses: number[],
  baseIndex: number,
): Color[] {
...
function getBezoldBruckeCompensation(
  hue: number,
...
function getLightnessProgression(
...
function findBaseColorPosition(
  baseLightness: number,
  progression: number[],
): number {
  let closestIndex = 0;
  ...
  const upper =
    closestIndex < progression.length - 1
      ? progression[closestIndex + 1] - 0.005
      : 1;
