...
export function generateTriadic(
  ...
  try {
    ...
    if (isAchromatic(baseColorObj))
      return generateNeutralPalette(baseColor, 6, "triadic", format);
    ...
    triadicHues.forEach((hue, triadIndex) => {
      if (triadIndex === 0) {
        ...
        const variations = isFirstTriad
          ? triadVariations.first
          : triadVariations.second;
    ...
    return buildPaletteColors(baseColor, finalColors, "triadic", format);
  } catch (e) {
