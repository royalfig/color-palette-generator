...
import {
  applyEnhancementsToSplitComplementary,
  avoidMuddyZones,
  polishPalette,
} from "./enhancer";
...
function getAdaptiveSplitComplementary(baseColor: Color): number[] {
  ...
  if (hue >= 30 && hue < 90) {
    ...
    return [
      hue,
      (hue + 160 + intensity * 12) % 360, // Deeper blues for more vivid oranges
      (hue + 200 + intensity * 8) % 360, // Dream purples
    ];
  }
...
function getLuminositySplitComplementary(baseColor: Color): number[] {
  ...
  return [
    hue,
    (hue + 165 + lightInfluence) % 360,
    (hue + 195 - lightInfluence) % 360,
  ];
}
...
export function generateSplitComplementary(
  ...
  try {
    ...
    switch (style) {
      case "square":
        splitHues = getMathematicalSplitComplementary(
    ...
    splitHues.forEach((hue, splitIndex) => {
      if (splitIndex === 0) {
        ...
        initialColors.push(
          applyVariation(baseColorObj, baseVariations.dark, hue),
        );
      } else {
        // Split complement colors + their variants
        const isFirstSplit = splitIndex === 1;
        const variations = isFirstSplit
          ? complementVariations.first
          : complementVariations.second;
        ...
        initialColors.push(
          applyVariation(baseColorObj, variations.pure, finalHue),
          applyVariation(baseColorObj, variations.muted, finalHue),
        );
      }
    });
    ...
    const finalColors = enhanced
      ? polishPalette(
          applyEnhancementsToSplitComplementary(initialColors, style, 0),
          0,
        ) // Base is at index 0
      : initialColors;

    return buildPaletteColors(
      baseColor,
      finalColors,
      "split-complementary",
      format,
    ...
    throw new Error(
      `Failed to generate split-complementary colors for ${baseColor}: ${e}`,
    );
  }
}
