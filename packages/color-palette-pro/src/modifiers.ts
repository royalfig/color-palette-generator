import { ColorSpace } from "./types/types";
...
function sineModifier(
  ...
  return palette.map((color, idx) => {
    ...
    const lightnessShift =
      Math.sin(wavePosition * 1.5 + modifier * 0.008) * lightnessIntensity;
    ...
    newColor.oklch.l = Math.max(
      0.05,
      Math.min(0.95, currentLightness + lightnessShift),
    );
    ...
    return colorFactory(
      ...
      color.code.split("-")[0],
      idx,
      color.colorSpace as ColorSpace,
      false,
    );
  });
}

function waveModifier(
  palette: BaseColorData[],
  modifier: number,
): BaseColorData[] {
  ...
  return palette.map((color, idx) => {
    ...
    newColor.oklch.c = Math.max(
      0,
      Math.min(0.4, currentChroma * chromaMultiplier),
    );
...
function blockModifier(
  ...
  return palette.map((color, idx) => {
    ...
    const hueShift =
      Math.sin(wavePosition + Math.PI * 0.25) * rawTriangle * hueAmplitude;
    const chromaShift =
      Math.cos(wavePosition + Math.PI * 0.5) * rawTriangle * chromaAmplitude;
...
export function paletteModulator(
  palette: BaseColorData[],
  modulateValues: number[],
): BaseColorData[] {
