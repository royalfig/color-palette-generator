...
export function getAccessibleVariant(
  ...
  const effectiveFloor =
    chromaFloor * Math.max(0, 1 - Math.abs((target.oklch.l ?? 0.5) - 0.5) * 2);
...
export function dampedSurfaceChroma(
  ...
  const proximity = Math.max(
    minProximity,
    Math.min(1, distanceFromExtreme / 0.5),
  );
...
export function surfaceChromaFor(
  ...
  return dampedSurfaceChroma(
    c,
    surfaceL,
    isDarkMode,
    0.012 * treatment.surfaceChromaScale,
    treatment.minProximityBoost,
  );
}

export function colorToCss(color: Color, format: ColorFormat): string {
  if (format === "hex") return color.to("srgb").toString({ format: "hex" });
  if (format === "rgb" || format === "srgb")
    return color.to("srgb").toString({ precision: 4 });
...
export function surfaceTreatmentFor(style?: PaletteStyle): SurfaceTreatment {
  return style
    ? (SURFACE_TREATMENT[style] ?? DEFAULT_TREATMENT)
    : DEFAULT_TREATMENT;
}
