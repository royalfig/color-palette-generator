...
export function generateSplitComplementary(
  ...
  try {
    ...
    if (isAchromatic(baseColorObj)) return generateNeutralPalette(baseColor, 6, 'split-complementary', format)
    ...
    splitHues.forEach((hue, splitIndex) => {
      if (splitIndex === 0) {
        ...
        initialColors.push(applyVariation(baseColorObj, baseVariations.dark, hue))
      } else {
    ...
    return buildPaletteColors(baseColor, finalColors, 'split-complementary', format)
  } catch (e) {
